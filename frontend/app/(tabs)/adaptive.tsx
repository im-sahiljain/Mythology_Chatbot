...
export default function AdaptiveScreen() {
  ...
  const send = async (text?: string) => {
    ...
    if (!t.trim()) return;
    if (!text) setInput('');
    const userMsg: Msg = { id: Date.now().toString(), role: 'user', content: t };
    const updated = [...msgs, userMsg];
    setMsgs(updated);
    setLoading(true);
    try {
      const turns = updated.filter(m => m.role === 'user').map(m => m.content).join(' -> ');
      const res = await apiService.completenessStrategy(turns);
      setMsgs(prev => [...prev, {
        id: (Date.now() + 1).toString(), role: 'assistant', content: res.reply,
        score: res.completeness_score, status: res.status,
        options: res.options || [], sources: res.sources || [],
      }]);
    } catch (e) { console.error(e); }
    finally { setLoading(false); }
  };

  const pick = (msgId: string, opt: string) => {
    setMsgs(prev => prev.map(m => m.id === msgId ? { ...m, picked: opt } : m));
    send(opt);
  };
        ...
        {msgs.length === 0 && (
          <FadeSlide>
            <View style={[st.empty, { backgroundColor: theme.surface, borderColor: theme.surfaceBorder }]}>
              <View style={[st.emptyIcon, { backgroundColor: theme.amberSubtle }]}>
                <Text style={{ fontSize: 26 }}>⚖️</Text>
              </View>
              <Text style={[st.emptyTitle, { color: theme.text, fontFamily: serif }]}>Adaptive Evaluator</Text>
              <Text style={[st.emptySub, { color: theme.textSecondary, fontFamily: body }]}>
                Describe your dilemma. The AI evaluates clarity and offers interactive refinement paths when needed.
              </Text>
            </View>
          </FadeSlide>
        )}
            ...
            {m.role === 'user' ? (
              ...
              <View style={[st.aiCard, { backgroundColor: theme.surface, borderColor: theme.surfaceBorder }]}>
                <View style={st.aiHead}>
                  <Text style={[st.aiLabel, { color: theme.accent, fontFamily: bold }]}>EVALUATOR</Text>
                  {m.score !== undefined && (
                    <View style={[st.scorePill, {
                      backgroundColor: m.score >= 0.7 ? theme.greenSubtle : theme.amberSubtle,
                      borderColor: m.score >= 0.7 ? theme.green : theme.amber,
                    }]}>
                      <Text style={[st.scoreText, {
                        color: m.score >= 0.7 ? theme.green : theme.amber,
                        fontFamily: bold,
                      }]}>{(m.score * 100).toFixed(0)}%</Text>
                    </View>
                  )}
                </View>
                <StreamingText text={m.content} />

                {m.status === 'needs_clarification' && m.options && m.options.length > 0 && !m.picked && (
                  <View style={st.chips}>
                    <Text style={[st.chipsLabel, { color: theme.textTertiary, fontFamily: bold }]}>REFINE</Text>
                    {m.options.map((opt, i) => (
                      <FadeSlide key={i} delay={i * 70} distance={10} from="left">
                        <Pressable onPress={() => pick(m.id, opt)}>
                          <View style={[st.chip, { backgroundColor: theme.accentSubtle, borderColor: theme.surfaceBorder }]}>
                            <View style={[st.chipDot, { backgroundColor: theme.accent }]} />
                ...
                {m.status === 'resolved' && m.sources && m.sources.length > 0 && <SourceCard sources={m.sources} />}
              </View>
            )}
          </FadeSlide>
        ))}
        ...
        {loading && (
          <FadeSlide duration={200}>
            <View style={[st.loader, { backgroundColor: theme.bgTertiary }]}>
              <TypingDots />
              <Text style={[st.loaderText, { color: theme.textSecondary, fontFamily: body }]}>Evaluating...</Text>
            </View>
          </FadeSlide>
        )}
      </ScrollView>
